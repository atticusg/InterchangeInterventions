...
from captum._utils.common import _select_targets, _format_inputs
...
def _run_forward(
    ...
    # extract hidden state and return it
    logits, hidden_state = output 
    return _select_targets(logits, target), hidden_state
#########################################################################################
    ...
    def attribute(
        ...
        def gradient_func(
            ...
            with torch.autograd.set_grad_enabled(True):
                ...
                model_output, hidden_state = output

                start_idx = START
                end_idx = START + INTERVENTION_H_DIM
                ...
                mask = torch.zeros_like(final_layer_gradient)
                # either go through mediated site, or go around it
                if self.is_mediated:
                    mask[:, start_idx:end_idx] = final_layer_gradient[:, start_idx:end_idx]
                else:
                    mask[:, :start_idx] = final_layer_gradient[:, :start_idx]
                    mask[:, end_idx:] = final_layer_gradient[:, end_idx:]

                grads = torch.autograd.grad(
                    outputs=torch.unbind(hidden_state),
                    inputs=inputs,
